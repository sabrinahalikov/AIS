...
and(edit6.Text <> '') and (edit7.Text <> '')and (edit8.Text <> '')and (edit9.Text <> '')and (edit10.Text <> '') then
  begin
    ADOQuery1.SQL.Text := 'INSERT INTO `pay` (`Адрес`, `Комнаты`, `Этажность`,`Этаж`, `Комплатеж`, `Короткие сроки аренды`, `Стоимость`, `Имя`, `Фамилия`, `Телефон`) ' +
...
begin
  ...
  begin
    ADOQuery1.SQL.Text := 'INSERT INTO `ipot` (`ФИО`, `Номер телефона`) VALUES (:Field1Value, :Field2Value)';
  ...
  try
    ...
    for i := 0 to nameTxt.Count - 1 do begin
      if nameTxt[i] = saveTxt[i] then begin
          FullName := nameTxt[i];
          Label29.Caption := 'Аккаунт: ' + FullName;
        end;
      end;
  finally
    nameTxt.Free;
